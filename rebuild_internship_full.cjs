...
let mainContent = mainMatch[1]; // Get inside of body

// Remove Script tag from HTML entirely (we will manually inject it into useEffect)
...
// Wrap everything inside a div with the body classes to preserve global styles (like bg-surface text-on-surface font-body-md)
// In the original HTML it was: <body class="bg-surface text-on-surface font-body-md text-body-md flex flex-col min-h-screen">
// We must prefix those too!
const bodyClasses = "bg-stitch-surface text-stitch-on-surface font-stitch-body-md text-stitch-body-md flex flex-col min-h-screen w-full";
mainContent = `<div className="${bodyClasses}">\n${mainContent}\n</div>`;
...
console.log("Completely rebuilt Internship.tsx with exact styling and exact HTML structure including header and footer!");
